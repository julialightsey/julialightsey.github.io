...
if object_id(N'[dbo].[sp__defragment__index]', N'P') is not null
  drop procedure [dbo].[sp__defragment__index]; ;
...
/*
	...
	------------------------------------------------
	declare @schema [sysname] = N'dbo', @object [sysname] = N'sp__defragment__index';
	--
...
*/
create procedure [dbo].[sp__defragment__index] @maximum_fragmentation    [int] = 25
                ...
                if @sql is not null
                  begin
                      ...
                      set @xml_builder = (select @schema                                                                               as N'@schema'
                                                 , @table                                                                              as N'@table'
                                                 , @index                                                                              as N'@index'
                                                 , [dm_db_index_physical_stats].[page_count]                                           as N'@page_count'
...
exec [sp_MS_marksystemobject] N'sp__defragment__index';

go

exec sys.sp_addextendedproperty @name         = N'revision_atmdna_18'
                                , @value      = N'lightseyk@ccculv.com - Created.'
                                , @level0type = N'schema'
                                , @level0name = N'dbo'
                                , @level1type = N'procedure'
                                , @level1name = N'sp__defragment__index'
                                , @level2type = null
                                , @level2name =null;
...
exec sys.sp_addextendedproperty @name         = N'execute_as'
                                , @value      = N'
DECLARE @maximum_fragmentation    [INT] = 15
        , @fillfactor             [INT] = NULL
        , @minimum_page_count     [INT] = 500
        , @reorganize_demarcation [INT] = 25
        , @defrag_count_limit     [INT] = 1000
        , @output                 [XML];
EXECUTE [dbo].[sp__defragment__index]
