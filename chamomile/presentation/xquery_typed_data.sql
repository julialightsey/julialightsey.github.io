...
go
/*
      select @test_count = @output.value('declare namespace chamomile="http://www.katherinelightsey.com/"; data(/chamomile:test_stack/@test_count)[1]', N'int')
             , @pass_count = @output.value('declare namespace chamomile="http://www.katherinelightsey.com/"; data(/chamomile:test_stack/@pass_count)[1]', N'int');
*/
...
if object_id(N'[utility_secure].[log]'
             , N'U') is not null
...
-------------------------------------------------
select [entry]
       , [entry].value(N'data (/*/description/text())[1]'
                       , N'[nvarchar](max)')
