/*
	...
	--	description
	---------------------------------------------
		[verbose].[lower_case].[verb__object_name].[english_grammar].[bracketed].[separated_by_underscore].[consistent].[singular]
		...
		[verb__object_name]			- You "get data" rather than "data get", and "generate extended properties" rather than 
									...
									- [verb__object] relationships are separated by double underscores to distinguish them 
										from a proper name. For example; for a table holding accounting records of sales of books 
										such as [accounting].[book_sale] you would have an accessor named 
										[accounting].[get__book_sale] rather than [accounting].[get_book_sale].
		[english_grammar]			- Use of standard english grammar syntax simplifies the naming of objects. Name 
										objects the same as you would say them in a sentence in writing or in conversation.
		[bracketed]					- Rather than only occasionally use brackets as required for reusing system 
										reserved words, with non-standard characters such as ".", be consistent and use brackets throughout.
		...
		[singular]					- To maintain consistency, I use singular names for all objects. For example, 
										[address] rather than [addresses]. 
