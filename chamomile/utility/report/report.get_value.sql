...
if object_id(N'[report].[get_value]'
             , N'FN') is not null
  drop function [report].[get_value];
...
/*
	...
	------------------------------------------------
	declare @schema [sysname] = N'report', @object [sysname] = N'get_value';
	--
	-------------------------------------------------
...
*/
create function [report].[get_value] (@id          [uniqueidentifier]
                                      , @category  [sysname]
                                      , @class     [sysname]
                                      , @type      [sysname]
                                      , @timestamp [datetime])
...
exec sys.sp_addextendedproperty
  @name = N'execute_as',
  @value = N'
	select [report].[get_value] (N''category'', N''class'', N''type'', null);',
  @level0type = N'schema',
  @level0name = N'report',
  @level1type = N'function',
  @level1name = N'get_value';
