...
if object_id(N'[metadata].[get_list]'
             , N'TF') is not null
  drop function [metadata].[get_list];
...
/*
	...
	------------------------------------------------
	declare @schema [sysname] = N'metadata', @object [sysname] = N'get_list';
...
*/
create function [metadata].[get_list] (@category    [sysname]
                                       , @class     [sysname]
                                       , @type      [sysname]
                                       , @timestamp [datetime])
returns @list table (
...
as
  begin
      ...
      select [id]
             ...
             , [entry]
             , [created]
      from   [repository_secure].[get_list] (N'metadata'
                                             , @category
                                             , @class
                                             , @type
                                             , @timestamp);
...
exec sys.sp_addextendedproperty
  @name = N'execute_as',
  @value = N'
		select * from [metadata].[get_list] (N''clinicScorecard'', N''patientExperience'', N''cfsParticipation'');',
  @level0type = N'schema',
  @level0name = N'metadata',
  @level1type = N'function',
  @level1name = N'get_list';
