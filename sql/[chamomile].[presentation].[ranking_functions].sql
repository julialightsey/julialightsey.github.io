...
with cte_deleteduplicates(c[flower], rnumber)
     as (select c[flower]
                , row_number()
                    over (
                      partition by c[flower]
                      order by c[flower]) as rnumber
         from   @duplicate)
...
-- 
if object_[id](N'tempdb..##flower', N'U') is not null
  drop table ##flower;

go

create table ##flower (
  [id]       int [id]entity(1, 1)
  , [flower] int
  , [color] datetime
  );
