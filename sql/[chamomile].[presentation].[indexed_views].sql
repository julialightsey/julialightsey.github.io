...
-- code block begin
-------------------------------------------------
if object_id (N'[presentation_indexed_view].[view_flower]', N'V') is not null
  drop view [presentation_indexed_view].[view_flower];
go
if object_id (N'[presentation_indexed_view].[flower]', N'U') is not null
...
-- code block begin
-------------------------------------------------
if indexproperty (object_id('[presentation_indexed_view].[view_flower]'), 'presentation_indexed_view.view_flower.flower.unique_clustered_index', 'IndexID') is not null
...
where  [indexes].object_id = object_id(N'[presentation_indexed_view].[flower]', N'U');
go 
...
if indexproperty (object_id('[presentation_indexed_view].[view_flower]'), 'presentation_indexed_view.view_flower.flower.unique_clustered_index', 'IndexID') is not null
  drop index [presentation_indexed_view.view_flower.flower.unique_clustered_index] on [presentation_indexed_view].[view_flower];
go
if indexproperty (object_id('[presentation_indexed_view].[view_flower]'), 'presentation_indexed_view.flower.view_flower.nonclustered_index_color_include_flower', 'IndexID') is not null
...
select [indexes].*
from   [sys].[indexes] as [indexes]
where  [indexes].[object_id] = object_id(N'[presentation_indexed_view].[view_flower]', N'V');
go
select *
from   [sys].[indexes] as [indexes]
where  [indexes].object_id = object_id(N'[presentation_indexed_view].[flower]', N'U');
...
insert into [presentation_indexed_view].[flower]
            ([flower],[color], [care])
values      (N'rose', N'red', N'lots of water'),
            (N'chamomile', N'yellow', N'just a little water');
select [flower],[color], [care]
...
-- code block begin
-------------------------------------------------
if object_id(N'[presentation_indexed_view].try_to_access_system_objects', N'V') is not null
...
while @i < 50000
  begin
      insert into [presentation_indexed_view].[flower]
                  ([flower],[color],[care])
      values      (N'rose_' + cast(@i as [sysname]),N'red',N'water');
      set @i = @i + 1;
  end;
--
